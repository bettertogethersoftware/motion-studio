...
| [timeline-footage-and-stills-plan.md](timeline-footage-and-stills-plan.md) | `+ image` as a scaffolded scene, and trimming footage — **revised 2026-08-08** to keep both outside the assemble path, the frame-count rule and the segment-kind walks. **§2 scene-from-image delivered 2026-08-08. §3's measurement taken the same day and it moved the design**: the re-encode is 0.50 s + 0.69 s per second *kept* (~105 s for the one real footage segment), but a **stream copy does the same trim in 0.2–1.8 s** — frame-exact, signature-preserving, `gopSize` is already in `neednotMatch` — whenever the footage was engine-prepared (0.13 s keyframe grid vs 0.9–3.3 s for as-supplied). So the drag commits on release for prepared footage, and offers a one-time prepare for the rest. §3 build is next |
