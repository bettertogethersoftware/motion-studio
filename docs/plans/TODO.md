...
| [timeline-footage-and-stills-plan.md](timeline-footage-and-stills-plan.md) | **COMPLETE 2026-08-08** — `+ image` as a scaffolded scene (§2) and footage trimming (§3), both kept outside the assemble path, the frame-count rule and the segment-kind walks. §3's measurement moved its own design: the re-encode is 0.50 s + 0.69 s per second *kept* (~105 s for the one real footage segment), but a **stream copy does the same trim in 0.1–1.8 s** — frame-exact, signature-preserving, `gopSize` already in `neednotMatch`, and proven by building a film and comparing decoded frames. So the handle commits on release and snaps to the clip's real keyframe grid. Two follow-ups deliberately left out, listed at the foot of the plan: a one-click "prepare this clip for finer trimming", and collecting superseded trim outputs |
